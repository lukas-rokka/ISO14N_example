...
matrix ISO14N(
  ...
  vector I_tot_ver_sh, vector I_tot_hor_sh,
  vector P_gn_sol,
  vector P_gn_int, 
  ...
  int N_pl, int debug, int Nout, int recalc_h_re) 
{
 ...
 /* Heat transfer coefs, resistances and heat capacities */
  ...
  matrix[m, m] A0 = rep_matrix(0, m, m);  // system coefs
  matrix[m, m] A_inv[N_U10];             // Array of inverted A
  matrix[N, (Nout > 2) ? (m + 5) : Nout] out; // returned values
  
  // AX + BU 
  matrix[m, m] A_c;
  matrix[m, m] A_arr[N_U10];
  matrix[m, 10] B = rep_matrix(0, m, 10);
  matrix[m, 10] B_arr[N_U10];
  vector[10] U;
  
  // Kalman filter
  int p = 1;       // number of observations
  int q = m;       // number of state disturbances
  vector[p] v;
  matrix[p, m] H;  // desing / measuremnt model matrix
  matrix[q, q] Q;  // state covariance
  matrix[p, p] R;  // observation covariance
  matrix[m, m] P;  // covariance
  matrix[m, p] K;  // Kalman gain
  matrix[p, p] S;  // F in Koopman
  ...
  *****************************************************/
  ...
  // Roof
  //B[idx_se[rf], 1] = r_el[rf]*h_se[3, rf] / C[idx_se[rf]]; // r_rf * h_se;rf * T_e
  B[idx_se[rf], 3] = r_el[rf]*Fsky_hor*h_re / C[idx_se[rf]]; // r_rf * h_re;sky * T_sky
  B[idx_se[rf], 4] = r_el[rf] * a_sol / C[idx_se[rf]];  // r_rf * a_sol * I_hor
  B[idx_si[rf], 6:8] = f_el[rf]*[f_r_sol, f_r_int, f_r_hyd] / C[idx_si[rf]]; // f_rf*[f_r_sol*P_sol;gl, ...]
  
  // External walls
  //B[idx_se[ew], 1] = r_el[ew]*h_se[3, ew] / C[idx_se[ew]]; // r_ew * h_se;ew * T_e
  B[idx_se[ew], 3] = r_el[ew]*Fsky_ver*h_re / C[idx_se[ew]]; // r_ew * h_re;sky * T_sky
  B[idx_se[ew], 5] = r_el[ew] * a_sol / C[idx_se[ew]];  // r_ew * a_sol * I_ver
  B[idx_si[ew], 6:8] = f_el[ew]*[f_r_sol, f_r_int, f_r_hyd] / C[idx_si[ew]]; // f_ew*[f_r_sol*P_sol;gl, ...]
  
  // Glazing
  //B[idx_se[gl], 1] = r_el[gl]*h_se[3, gl] / C[idx_se[gl]]; // r_gl * h_se;gl * T_e
  B[idx_se[gl], 3] =  r_el[gl]*Fsky_ver*h_re/ C[idx_se[gl]]; 
  B[idx_si[gl], 6:8] = f_el[gl]*[f_r_sol, f_r_int, f_r_hyd] / C[idx_si[gl]]; // f_gl*[f_r_sol*P_sol;gl, ...]
  
  // Internal mass
  B[idx_si[im], 6:8] = f_el[im]*[f_r_sol, f_r_int, f_r_hyd] / C[idx_si[im]]; // f_im*[f_r_sol*P_sol;gl, ...]
  
  // Ground floor
  B[idx_se[gf], 2] = H_gr_vi / C[idx_se[gf]]; // r_gf * h_gr_vi * T_e
  B[idx_si[gf], 6:8] = f_el[gf]*[f_r_sol, f_r_int, f_r_hyd] / C[idx_si[gf]]; // f_gf*[f_r_sol*P_sol;gl, ...]
  
  // Internal air node
  B[m, 1] = H_tb / C[m];  // H_tb * T_e 
  B[m, 9:10] = [1.0, 1.0] / C[m];  // [P_ve, P_inf] 
  B[m, 6:8] = [f_c_sol, f_c_int, f_c_hyd] / C[m];  // [f_c_sol*P_sol;gl, ...]
  ...
  *****************************************************/
  ...
  for (el in 1:N_el) {
    // Exterior side
    A0[idx_se[el], idx_se[el]]   =  H_el[el, 1]; // diagonal/capacity node
    A0[idx_se[el], idx_se[el]+1] = -H_el[el, 1]; // coupling next node
    // middle/inside nodes
    if (pln[el] > 2) {
      for (pl in 2:(pln[el]-1)) {
        int idx_diag = idx_se[el]+pl-1;
        A0[idx_diag, idx_diag-1] = -H_el[el, pl-1]; // coupling previous node
        A0[idx_diag, idx_diag] =  + H_el[el, pl-1] + H_el[el, pl]; // diagonal/capacity node
        A0[idx_diag, idx_diag+1] = -H_el[el, pl];  // coupling next node
      }
    }
    // Interior side node
    A0[idx_si[el], idx_si[el]-1] = -H_el[el, pln[el]-1]; // coupling previous node
    A0[idx_si[el], idx_si[el]]   =  H_el[el, pln[el]-1] + H_ci[el] + (1-f_el[el])*H_ri[el]; // diagonal/capacity node
    
    // interior surface radiative heat exchange
    for (j in 1:N_el) if (j != el) A0[idx_si[el], idx_si[j]] = -H_ri[el]*f_el[j];
  }

  // Internal air thermal zone node
  A0[m, m] = sum(H_ci) + H_tb;
  ...
  A0[idx_se[5], idx_se[5]] += H_gr_vi;
  
  // precalculate inverse A matrises for every U10 category, using
  // variable H_se for roof, external walls and glazing
  Cdt = C/dt;
  A_c = A0 + diag_matrix(Cdt);
  for (i in 1:N_U10) {
    A_c[idx_se[rf], idx_se[rf]] = Cdt[idx_se[rf]] + H_el[rf, 1] + r_el[rf]*(h_se[i, rf] + Fsky_hor*h_re);
    A_c[idx_se[ew], idx_se[ew]] = Cdt[idx_se[ew]] + H_el[ew, 1] + r_el[ew]*(h_se[i, ew] + Fsky_ver*h_re);
    A_c[idx_se[gl], idx_se[gl]] = Cdt[idx_se[gl]] + H_el[gl, 1] + r_el[gl]*(h_se[i, gl] + Fsky_ver*h_re);
    A_inv[i] = inverse_spd(A_c);
  }
  
  for (i in 1:m) A_c[i, ] = -A0[i, ] / C[i];
  for (n in 1:N_U10) {
    A_c[idx_se[rf], idx_se[rf]] = -(H_el[rf, 1] + r_el[rf]*(h_se[n, rf] + Fsky_hor*h_re))/C[idx_se[rf]];
    A_c[idx_se[ew], idx_se[ew]] = -(H_el[ew, 1] + r_el[ew]*(h_se[n, ew] + Fsky_ver*h_re))/C[idx_se[ew]];
    A_c[idx_se[gl], idx_se[gl]] = -(H_el[gl, 1] + r_el[gl]*(h_se[n, gl] + Fsky_ver*h_re))/C[idx_se[gl]];
    B[idx_se[rf], 1] = r_el[rf]*h_se[n, rf] / C[idx_se[rf]]; 
    B[idx_se[ew], 1] = r_el[ew]*h_se[n, ew] / C[idx_se[ew]]; 
    B[idx_se[gl], 1] = r_el[gl]*h_se[n, gl] / C[idx_se[gl]]; 
    A_arr[n] = matrix_exp(A_c*dt);
    B_arr[n] = (A_arr[n] - diag_matrix(rep_vector(1.0, m))) / A_c * B;
  }
  ...
  *****************************************************/
  ...
  if (debug > 3) {
    for (i in 1:m) print(round(A0[i, 1:m]*100)/100);
    print("m: ", m, ", N: ", N);
    print("idx_si:", idx_si);
    print("idx_se:", idx_se);
    print("r_el: ", r_el);
  ...
  *****************************************************/
  ...
  for (t in 1:N) {
    ...
    *****************************************************/
    ...
    U[7] = P_gn_int[t];
    U[8] = P_hyd[t];
    U[9] = (H_inf[t])*(T_e[t] - X[m]);
    U[10] = (H_ve[t])* (T_e[t] - X[m]);
    
    
    /*****************************************************
    // Time update (the effect of system dynamics) 
    *****************************************************/
    X = A_arr[U10_idx[t]]*X + B_arr[U10_idx[t]]*U;
     
    // solve the equation system. A_inv[U10_idx[t]] are pre-calculated
    // X = mdivide_left_ldlt(A, b)';  // mdivide_left_ldlt doesn't exist yet, should be faster than mdivide_left_spd
    ...
    *****************************************************/
    ...
    if (Nout==1) {
      ...
      out[t, m+2] = 0.030*(X[idx_se[rf]] + T_sky[t]);//u_trv[t];                     // control signal radiators
